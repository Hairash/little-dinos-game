...
from game.services.move_validation import validate_move, apply_move_to_cell
from game.services.visibility import calculate_visibility, filter_field_for_player
...
def apply_move_txn(game_code: str, user_id: int, payload: dict, client_seq: int):
    ...
    with transaction.atomic():
        ...
        is_valid, error_msg = validate_move(
            field_for_validation, width, height, from_coords, to_coords, player_order, enable_scout_mode
        )
        if not is_valid:
        ...
        building_captured, cells_changed = apply_move_to_cell(
            field, from_coords[0], from_coords[1], to_coords[0], to_coords[1], 
            player_order, settings
        )
        ...
        winner_order = check_game_ended(field, width, height, len(GamePlayer.objects.filter(game=game)))
    ...
    with transaction.atomic():
        ...
        for x in range(width):
            for y in range(height):
                ...
                if (unit and unit.get('player') == player_order and
                    building and building.get('_type') == BUILDING_TYPES['OBELISK']):
        ...
        if not has_unit_on_obelisk:
            return False, {"code": "NO_OBELISK", "msg": "You must have a unit on an obelisk to scout"}
        ...
        normal_visibility = calculate_visibility(
            field, width, height, player_order, fog_of_war_radius,
            settings.get('enableFogOfWar', True), None  # Don't include scout coords yet
        )
