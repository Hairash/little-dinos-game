...
def get_wave_field(field, width, height, enable_scout_mode=True):
    ...
    for x in range(width):
        ...
        for y in range(height):
            ...
            is_passable = (
                terrain_kind == TERRAIN_TYPES["EMPTY"]
                and not has_unit
                and not (enable_scout_mode and is_hidden)
            )
    ...
    while wave:
        ...
        for neighbour in neighbours:
            ...
            if cur_x == x1 and cur_y == y1:
                ...
                return True
            if (cur_x, cur_y) not in visited and (
                wave_field[cur_x][cur_y] is None or wave_field[cur_x][cur_y] > s
            ):
...
def validate_move(
    field, width, height, from_coords, to_coords, player_order, enable_scout_mode=False
):
    """
    ...
    if x0 < 0 or x0 >= width or y0 < 0 or y0 >= height:
        return (
            False,
            f"Source coordinates out of bounds: ({x0},{y0}) not in [0-{width-1}],[0-{height-1}]",
        )
    if x1 < 0 or x1 >= width or y1 < 0 or y1 >= height:
        return (
            False,
            f"Destination coordinates out of bounds: ({x1},{y1}) not in [0-{width-1}],[0-{height-1}]",
        )
    ...
    if x0 >= len(field) or y0 >= len(field[x0]):
        return (
            False,
            f"Source cell out of field structure: field has {len(field)} columns, column {x0} has {len(field[x0]) if x0 < len(field) else 0} rows",
        )
    if x1 >= len(field) or y1 >= len(field[x1]):
        return (
            False,
            f"Destination cell out of field structure: field has {len(field)} columns, column {x1} has {len(field[x1]) if x1 < len(field) else 0} rows",
        )
    ...
    if unit.get("player") != player_order:
        return (
            False,
            f"Unit does not belong to player: unit.player={unit.get('player')}, expected={player_order}",
        )
        ...
        if max_bases == 0:  # No limit
            ...
            for x in range(len(field)):
                for y in range(len(field[x])):
                    ...
                    if b:
                        if (
                            b.get("_type") == BUILDING_TYPES["BASE"]
                            and b.get("player") == player_order
                        ):
                            bases_count += 1
