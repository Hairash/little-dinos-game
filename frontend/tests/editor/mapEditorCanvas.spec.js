...
describe('MapEditorCanvasPage', () => {
  ...
  describe('keyboard shortcuts', () => {
    // Build a keydown-like event; defaults to a plain (non-field) target
    // with no modifiers.
    const ev = (key, over = {}) => ({
      key,
      target: { tagName: 'DIV' },
    ...
    it('T/B/U/E select the matching tool', () => {
      const w = makeWrapper()
      w.vm.handleKeydown(ev('b'))
      expect(w.vm.activeTool).toBe('building')
      w.vm.handleKeydown(ev('u'))
      expect(w.vm.activeTool).toBe('dino')
      w.vm.handleKeydown(ev('e'))
      expect(w.vm.activeTool).toBe('eraser')
      w.vm.handleKeydown(ev('t'))
      expect(w.vm.activeTool).toBe('terrain')
    })

    it('is case-insensitive (Shift+T still selects terrain)', () => {
      const w = makeWrapper()
      w.vm.activeTool = 'dino'
      w.vm.handleKeydown(ev('T'))
      expect(w.vm.activeTool).toBe('terrain')
    ...
    it('M toggles the move flow on and off', () => {
      const w = makeWrapper()
      w.vm.handleKeydown(ev('m'))
      expect(w.vm.moveState).toEqual({ stage: 'corner1' })
      w.vm.handleKeydown(ev('m'))
      expect(w.vm.moveState).toBeNull()
    })
    ...
    it('= zooms in and - zooms out', () => {
      const w = makeWrapper()
      w.vm.cellSize = 40
      w.vm.handleKeydown(ev('='))
      expect(w.vm.cellSize).toBe(50)
      w.vm.handleKeydown(ev('-'))
      expect(w.vm.cellSize).toBe(40)
    })

    it('Z undoes the last change', () => {
      const w = makeWrapper()
      w.vm.activeTool = 'dino'
      w.vm.toolConfig.dino = { player: 0, speed: 2 }
      w.vm.onPaintStart(3, 3)
      expect(w.vm.map.field[3][3].unit).not.toBeNull()
      w.vm.handleKeydown(ev('z'))
      expect(w.vm.map.field[3][3].unit).toBeNull()
      expect(w.vm.undoSnapshot).toBeNull()
    })

    it('"." toggles the gear menu', () => {
      const w = makeWrapper()
      expect(w.vm.gearOpen).toBe(false)
      w.vm.handleKeydown(ev('.'))
      expect(w.vm.gearOpen).toBe(true)
      w.vm.handleKeydown(ev('.'))
      expect(w.vm.gearOpen).toBe(false)
    })
    ...
    it('ignores shortcuts while typing in a form field', () => {
      const w = makeWrapper()
      w.vm.activeTool = 'terrain'
      w.vm.handleKeydown(ev('b', { target: { tagName: 'INPUT' } }))
      expect(w.vm.activeTool).toBe('terrain')
    })

    it('ignores shortcuts with a modifier held (Cmd/Ctrl)', () => {
      const w = makeWrapper()
      w.vm.activeTool = 'terrain'
      w.vm.handleKeydown(ev('b', { metaKey: true }))
      w.vm.handleKeydown(ev('b', { ctrlKey: true }))
      expect(w.vm.activeTool).toBe('terrain')
    })

    it('ignores canvas shortcuts while the gear menu is open', () => {
      const w = makeWrapper()
      w.vm.activeTool = 'dino'
      w.vm.gearOpen = true
      w.vm.handleKeydown(ev('t'))
