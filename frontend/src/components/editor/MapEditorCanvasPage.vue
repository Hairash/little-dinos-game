      ...
      <div class="inner-info-panel">
        ...
        <span class="info-block right-group">
          ...
          <button
            ...
            title="Terrain (T) — left-click to select, right-click to change kind"
          ...
          >
            <button
              class="infoBtn toolBtn"
              title="Building (B) — left-click to select, right-click to change type"
          ...
          >
            <button
              class="infoBtn toolBtn"
              title="Dino (U) — left-click to select, right-click to change speed"
          ...
          <button
            ...
            title="Eraser (E) — left-click to select"
...
export default {
  ...
  methods: {
    ...
    // Keyboard shortcuts for the editor. Keyboard-only, so nothing here
    // ever fires on touch. Layout:
    //   M move · E eraser · U unit · B building · T terrain
    //   Z undo · = / + zoom in · - zoom out · . toggle menu · Esc cancel move
    handleKeydown(e) {
      // Esc cancels an in-progress move (same as re-tapping Move); it's
      // the one shortcut allowed to fire while a dialog/field has focus.
      if (e.key === 'Escape') {
        if (this.moveState) this.cancelMove()
        return
      }
      // Never hijack typing in the name/description/number fields, and
      // leave modifier combos (Cmd+Z, Ctrl+-, browser zoom, …) alone.
      ...
      // "." toggles the gear menu (open ⇄ close). Blocked only while a
      // confirm dialog is up so it can't yank the menu out from under it.
      if (key === '.') {
        if (this.confirmExit || this.confirmDropPlayers) return
        if (this.gearOpen) this.closeGearMenu()
        else this.gearOpen = true
        e.preventDefault()
        return
      }

      // The rest act on the canvas — ignore them while any overlay is
      // open so they don't fire "underneath" a menu/dialog.
      if (
        this.gearOpen ||
        this.resizeOpen ||
        this.playersOpen ||
        this.confirmExit ||
        this.confirmDropPlayers
      ) {
        return
      }
      ...
      switch (key) {
        ...
        case 'e':
          ...
          break
        case 'u':
          this.selectTool('dino')
          break
        case 'b':
          this.selectTool('building')
          break
        case 't':
      ...
      if (handled) {
        // A handled shortcut also dismisses any transient floating UI so
        // a stale tool popup / cell menu doesn't linger over the change.
        this.popup = null
        this.cellMenu = null
        this.cellPicker = null
        e.preventDefault()
