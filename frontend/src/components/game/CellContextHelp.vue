<template>
  <div v-if="visible" class="cell-context-help" :style="positionStyle">
    <!-- Refactored to avoid v-html for XSS safety -->
    <div v-if="cellContent">
      <b :style="cellContent.titleStyle">{{ cellContent.title }}</b>
      <template v-if="cellContent.description"> <br />{{ cellContent.description }} </template>
...
export default {
  ...
  computed: {
    ...
    positionStyle() {
      ...
      if (rightSideLeft + tooltipWidth <= maxX) {
        ...
        const top = Math.max(
          0,
          Math.min(cellY + this.cellSize / 2 - tooltipHeight / 2, maxY - tooltipHeight)
        )
    ...
    cannotBeCaptured() {
      ...
      if (
        !this.cell ||
        !this.cell.building ||
        this.cell.building._type !== Models.BuildingTypes.BASE
      ) {
        return false
      }
  ...
  methods: {
    ...
    buildingContent(_type) {
      ...
      const playerColor =
        building && building.player !== null && building.player !== undefined
