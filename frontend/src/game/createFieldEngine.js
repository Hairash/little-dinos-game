...
import { scaledBuildingsRange } from '@/game/const'

function pickBuildingsNum(rate, area) {
  const [min, max] = scaledBuildingsRange(rate, area)
  if (max === 0) return 0
...
class CreateFieldEngine {
  ...
  generateField() {
    ...
    for (let x = 0; x < this.width; x++) {
      ...
      for (let y = 0; y < this.height; y++) {
        ...
        const terrain =
          r > CreateFieldEngine.MOUNTAIN_RATIO
            ? Models.TerrainTypes.MOUNTAIN
            : Models.TerrainTypes.EMPTY
    ...
    for (let player = 0; player < this.playersNum; player++) {
      ...
      field[x][y].building = new Models.Building(player, Models.BuildingTypes.BASE)
    ...
    for (let buildingType in this.buildingRates) {
      ...
      for (let buildingCtr = 0; buildingCtr < buildingsNum; buildingCtr++) {
        ...
        while (
          field[x][y].terrain.kind === Models.TerrainTypes.MOUNTAIN ||
          !this.noBuildingsInDistance(field, x, y, minDistance)
        ) {
          x = Math.floor(Math.random() * this.width)
          y = Math.floor(Math.random() * this.height)
          failCtr++
  ...
  getSector(x, y) {
    return [
      Math.floor((x * this.sectorsNum) / this.width),
      Math.floor((y * this.sectorsNum) / this.height),
    ]
  }

  validateSector(x, y, sectors) {
    // console.log(x, y, sectors);
    if (!(x === 0 || x === this.sectorsNum - 1 || y === 0 || y === this.sectorsNum - 1))
      return false
    ...
    while (queue.length > 0) {
      ...
      for (let neighbour of neighbours) {
        ...
        if (field[x][y].terrain.kind === Models.TerrainTypes.MOUNTAIN)
          wField[x][y] = Math.min(wField[x][y], wField[curX][curY] + 1)
