...
export function createPlayers(humanPlayersNum, botPlayersNum) {
  let players = Array.from(
    { length: humanPlayersNum },
    () => new Models.Player(Models.PlayerTypes.HUMAN)
  )
  players = players.concat(
    Array.from({ length: botPlayersNum }, () => new Models.Player(Models.PlayerTypes.BOT))
  )
  // TODO: Shuffle it
  return players
}
...
export function createNewUnit(
  ...
  // );
  const movePoints =
    minSpeed + Math.floor(Math.random() * (maxSpeed - minSpeed + 1)) + speedModifier
  ...
  return field.map(col => {
    ...
    return col.map(cellData => {
      ...
      if (cellData.building) {
        cell.building = new Models.Building(cellData.building.player, cellData.building._type)
      }
