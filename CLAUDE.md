...
`.claude/docs/` contains deeper docs maintained alongside the code: `architecture.md`, `game-engines.md`, `game-components.md`, `game-rules.md`, `mixins.md`, `sub-menu-pages.md` (back-arrow / title layout pattern + confirm-dialog usage for sub-menu pages — read this before adding a new full-screen page outside the in-game view), `scenarios.md` (pre-designed single-player maps — builder helpers, engine-level rules like "fog blocks movement" and "only bases have owners", and how to add a new scenario), `troubleshooting.md`, `deployment-guide.md`, `security-guidelines.md`, `api/`. Also: `LINTING.md` (lint ordering rationale), `HOWTO.md` (first-time setup).
